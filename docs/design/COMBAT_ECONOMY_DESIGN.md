...
5. **Offline sim fidelity**: ~~offline runs slot-order with infinite mana~~ CORRECTED (E20 sprint, 2026-07-12): that line was never implemented — `offlineSim.ts` runs `runCombatTick` on the real state with zero mana special-casing. Offline safety instead rests on the E20 mana claim being a stateless pure function of (policy, state), so offline inherits it exactly; the claim fix repairs offline earnings for starved characters too.
...
## 9. E20 — Mana identity split + claims (OWNER DECISION + judged sprint, 2026-07-12)

**Owner decision:** martials (berserker / assassin / hunter) DROP mana entirely — their pacing is class ledgers + cooldowns + GCD ("warrior gets rage, assassin gets energy" was already true: the Phase 3 ledgers ARE the class resources). Casters (sorcerer / witchdoctor) KEEP mana as sustained-output pacing.

**Mechanism shipped:**
1. `ManaConfig.usesMana` (src/types/mana.ts) — false for berserker/assassin/hunter. `classUsesMana()` zeroes `skillManaCost` in tick.ts (one site feeds the gate + all three deduction sites) and passes `snap.mana = undefined` into `evaluatePolicy`, killing the gate, `minMana`, and claims for martials. PlayerHpBar hides the mana bar for them.
2. **Mana claims** (judged sprint winner, prototype-verified by both adversarial judges against the full gate suite): in `evaluatePolicy`, the first rule blocked ONLY by mana claims the pool iff regen closes the deficit within `MANA_CLAIM.maxHoldSec` (12) and cost ≤ max; while a claim is open, all lower-priority PAID rules hold (hard priority hold; autos still fire); cost-0 and `ignoreReserve` rules pass. Stateless per evaluation → offline-exact. `maxHoldSec = 0` is the kill-switch (byte-identical fall-through).
3. **GATE E6** (sim/qa-mana-recovery.ts, hard gate): caster claims recovery (wand: slot-order first Void Blast ≤ 11s measured from mana=0/regen 2 — was NEVER; preset arms ledger-bound not mana-bound), martial mana-free (berserker 31 Crushing Blows from perma-0 mana — was impossible with startFull:false/regen 0), kill-switch contrast (31 spends vs 0, same seed).

**Why the bug existed:** evaluatePolicy fell through unaffordable rules, so at OOM the cheapest skill always intercepted the regen trickle (measured: 0 assassinates in 300s at regen 2/s on BOTH arms; the kit fixture's natural regen 10/s is why every earlier gate missed it). Bar priority order IS the mana-allocation policy under fall-through — spender-first recovered spends but starved the ledger; no ordering fixes both. Claims arbitrate by rule priority instead.

**Residual (VERDICT LOG):** at regen 2 a spender-first blind bar can out-bank the tempo preset (its minMana builder guards never claim) — off-spec regime, optional data-only preset banking rule deferred behind its own re-gate. Q7 (free buff skills) verified still-open: buffs bypass evaluatePolicy's kind==='active' gate and have no deduction path — costing them is inert data until the skillStore buff path gets engine wiring.
