...
import { tickComboStates, createComboState } from '../combo';
...
export const daggerModule: WeaponModule = {
  ...
  preRoll(ctx: PreRollContext): PreRollResult {
    ...
    // Wave E2 (ask 8): the combo consume fold is shared runtime now —
    // dagger layers its rawBehaviors extras on top of the result.
    const combo = comboConsumePreRoll(ctx);
    let {
      damageMult, critChanceBonus, critMultiplierBonus, ailmentPotency,
      cdRefundPercent, counterDamageMult,
    } = combo;
    const {
      comboStates, guaranteedCrit, splashPercent, extraChains, burstDamage,
      focusBurst, markPassthrough, cdAcceleration, advanceOtherCooldownsSec,
  ...
  postCast(ctx: PostCastContext): PostCastResult {
    ...
    // Wave E2 (ask 8): combo state creation is shared runtime now
    // (creator arrays, trigger/minTargetsHit/icd gates, graphMod merges).
    comboStates = comboCreatePostCast({ ...ctx, comboStates });
