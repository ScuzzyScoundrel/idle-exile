...
export default function MobSelector({ zoneId, targetedMobId, mobKillCounts, onTargetMob }: {
  ...
  const [collapsed, setCollapsed] = useState(() =>
    localStorage.getItem('ie-mobs-collapsed') !== 'false'
  );
  const toggleCollapsed = () => {
    setCollapsed(prev => {
      localStorage.setItem('ie-mobs-collapsed', String(!prev));
      return !prev;
    });
  };
  ...
  return (
    <div className="bg-gray-800/60 rounded-lg border border-gray-700 p-2 space-y-1">
      <button
        ...
        className="w-full flex items-center gap-1 text-xs text-gray-400 font-semibold hover:text-gray-200 transition-colors"
      >
        <span className="text-[10px]">{collapsed ? '\u25B8' : '\u25BE'}</span>
        Target Mob
        {collapsed && targetedMobId && (
          <span className="ml-1 text-amber-400 font-normal">
            ({zoneMobs.find(m => m.id === targetedMobId)?.name ?? 'targeted'})
          </span>
        )}
      </button>
      {!collapsed && (
